 ...
 * MockHero MCP Server
 *
 * Exposes MockHero's API as MCP tools for AI agents:
 * - generate_test_data: Generate realistic test data from schema or plain English
 * - detect_schema: Convert SQL CREATE TABLE or JSON sample into MockHero schema
 * - list_field_types: List all 157 supported field types
 * - list_templates: List pre-built schema templates (e-commerce, blog, SaaS, social)
 * - generate_from_template: Generate data using a pre-built template
 *
 * Usage:
 ...
 *         "env": { "MOCKHERO_API_KEY": "mh_live_xxx" }
 *       }
 *     }
 *   }
 */
...
and names, emails, phones automatically match each row's nationality.`,
  {
    ...
    tables: z
      ...
      .optional()
      .describe("Structured schema — array of table definitions"),
    ...
    locale: z
      .string()
      .optional()
      .describe("Default locale (en, de, fr, es, ja, etc.). Auto-detected from country field if present."),
    seed: z
      .number()
      .optional()
      .describe("Seed for reproducible output. Same seed + same schema = identical data."),
  },
  async (params) => {
