...
export async function POST(request: Request) {
  if (!isAllowedOrigin(request.headers.get("origin"))) {
    return jsonResponse(
      request,
      {
        ...
        error: {
          code: -32000,
          message: "Origin is not allowed for MockHero Agent MCP",
        },
      },
      403
    );
  ...
  const response = await handleMcpMessage(request, body, { surface: "agent" });
  if (response === null) {
...
export async function GET(request: Request) {
  return Response.json(
    {
      error: "MockHero Agent MCP does not expose a server-initiated SSE stream. Use POST with JSON-RPC messages.",
    },
    {
      status: 405,
      headers: {
